import {ChapterIndexType, ChapterInterface} from './index';

const Chapter3: {[ChapterIndexType.Chapter3]: ChapterInterface } = {    
    chapter3: {
        chapterNameHindi: 'कर्मयोग',
        chapterNameEnglish: 'karma Yog',
        chapterDescription: '',
        totalSlokas: 43,
        slokas: [
            {
                slokaNumber: 1,
                uvach: "अर्जुन उवाच।",
                sanskritSloka: ["ज्यायसी चेत्कर्मणस्ते मता बुद्धिर्जनार्दन ।", "तत्किं कर्मणि घोरे मां नियोजयसि केशव ।।3।।"],
                romanSloka: ["jyāyasī cet karmaṇas te matā buddhir janārdana", "tat kiṁ karmaṇi ghore māṁ niyojayasi keśava"],
                wordsMeaning: [
                    {
                        sanskrit: "अर्जुनः उवाच",
                        meaning: "अर्जुन ने कहा",
                    },
                    {
                        sanskrit: "ज्यायसी, चेत्",
                        meaning: "श्रेष्ठ, यदि",
                    },
                    {
                        sanskrit: "कर्मण, ते",
                        meaning: "कर्मफल से, आप द्वारा",
                    },
                    {
                        sanskrit: "मता, बुद्धि",
                        meaning: "मानना, बुद्धि",
                    },
                    {
                        sanskrit: "जनार्दन",
                        meaning: "जीवों का पालन करने वाले, श्रीकृष्ण",
                    },
                    {
                        sanskrit: "तत्, किम, कर्मणि",
                        meaning: "तब, क्यों, कर्म",
                    },
                    {
                        sanskrit: "घोर, मम्",
                        meaning: "भयंकर, मुझे",
                    },
                    {
                        sanskrit: "नियोजयसि",
                        meaning: "लगाते हो",
                    },
                    {
                        sanskrit: "केशव",
                        meaning: "केशी नामक राक्षस का वध करने वाले, श्रीकृष्ण",
                    }
                ],
                meaningInHindi: "अर्जुन ने कहा! हे जनार्दन, यदि आप कर्म की अपेक्षा ज्ञान श्रेष्ठ मानते हैं तो फिर हे केशव! मुझे इस भयंकर कर्म में क्यों लगाते हैं?",
                meaningInEnglish: "Arjuna said: O Janārdana, O Keśava, if it is Your opinion that wisdom is better than action, then why do You wish to engage me in such violent activities?",
                meaningInGujrati: "",
                meaningInMarathi: "",
            },
            {
                slokaNumber: 2,
                uvach: "",
                sanskritSloka: ["व्यामिश्रेणेव वाक्येन बुद्धिं मोहयसीव मे ।", "तदेकं वद निश्चित्य येन श्रेयोऽहमाप्नुयाम् ॥2॥"],
                romanSloka: ["vyāmiśreṇeva vākyena buddhiṁ mohayasīva me", "tad ekaṁ vada niścitya yena śreyo’ham āpnuyām"],
                wordsMeaning: [
                    {
                        sanskrit: "व्यामिश्रेण",
                        meaning: "तुम्हारे अनेकार्थक शब्दों का",
                    },
                    {
                        sanskrit: "इव, वाक्येन",
                        meaning: "मानो, वचनों से",
                    },
                    {
                        sanskrit: "बुद्धिम्",
                        meaning: "बुद्धि",
                    },
                    {
                        sanskrit: "मोहयसि",
                        meaning: "मैं मोहित हो रहा हूँ",
                    },
                    {
                        sanskrit: "इव, मे, तत्",
                        meaning: "मानो, मेरी, उस",
                    },
                    {
                        sanskrit: "एकम्",
                        meaning: "एकमात्र",
                    },
                    {
                        sanskrit: "वद",
                        meaning: "अवगत कराए",
                    },
                    {
                        sanskrit: "निश्चित्य",
                        meaning: "निश्चित रूप से",
                    },
                    {
                        sanskrit: "येन",
                        meaning: "जिससे",
                    },
                    {
                        sanskrit: "श्रेयः",
                        meaning: "अति श्रेष्ठ",
                    },
                    {
                        sanskrit: "अहम्, आप्नुयाम्",
                        meaning: "मैं, प्राप्त कर सकू",
                    }
                ],
                meaningInHindi: "आपके अनेकार्थक वचनों से मेरी बुद्धि भ्रमित हो गयी है। कृपया मुझे निश्चित रूप से कोई एक ऐसा मार्ग बताएँ जिससे मैं कल्याणको प्राप्त कर सकूँ।",
                meaningInEnglish: "Your words seem to be contradictory and my mind is now confused. Therefore, kindly tell me which particular path is most beneficial for me.",
                meaningInGujrati: "",
                meaningInMarathi: "",
            },
            {
                slokaNumber: 3,
                uvach: "श्रीभगवानुवाच ।",
                sanskritSloka: ["लोकेऽस्मिन्द्विविधा निष्ठा पुरा प्रोक्ता मयानघ ।", "ज्ञानयोगेन साङ्ख्यानां कर्मयोगेन योगिनाम् ॥3॥"],
                romanSloka: ["loke’smin dvi-vidhā niṣṭhā purā proktā mayānagha", "jñāna-yogena sāṅkhyānāṁ karma-yogena yoginām"],
                wordsMeaning: [
                    {
                        sanskrit: "श्रीभगवान् उवाच",
                        meaning: "भगवान(श्री कृष्ण) ने कहा",
                    },
                    {
                        sanskrit: "लोके, अस्मिन्",
                        meaning: "संसार में, इस",
                    },
                    {
                        sanskrit: "द्वि-विधा, निष्ठा",
                        meaning: "दो प्रकार की, श्रद्धा",
                    },
                    {
                        sanskrit: "पुरा, प्रोक्ता",
                        meaning: "पहले, वर्णित",
                    },
                    {
                        sanskrit: "मया",
                        meaning: "मेरे(श्रीकृष्ण) द्वारा",
                    },
                    {
                        sanskrit: "अनघ",
                        meaning: "निष्पाप",
                    },
                    {
                        sanskrit: "ज्ञानयोगेन",
                        meaning: "ज्ञानयोग के द्वारा",
                    },
                    {
                        sanskrit: "सांख्यानाम्",
                        meaning: "ज्ञानियों का",
                    },
                    {
                        sanskrit: "कर्मयोगेन",
                        meaning: "कर्म योग के द्वारा",
                    },
                    {
                        sanskrit: "योगिनाम्",
                        meaning: "योगियों का",
                    }
                ],
                meaningInHindi: "भगवान ने कहा, हे निष्पाप अर्जुन! मैं पहले ही दो प्रकार की निष्ठा (मार्गों) का वर्णन कर चुका हूँ। उनमें ज्ञानियों की निष्ठा ज्ञानयोगसे और योगियों की निष्ठा कर्मयोगसे होती है।",
                meaningInEnglish: "Bhagavān Śrī Kṛṣṇa said: O faultless one, previously I explained the two paths found in this world – the path of wisdom for the empirical philosophers and the path of action for those that must perform action.",
                meaningInGujrati: "",
                meaningInMarathi: "",
            },
            {
                slokaNumber: 4,
                uvach: "",
                sanskritSloka: ["न कर्मणामनारम्भान्नैष्कर्म्यं पुरुषोऽश्नुते ।", "न च संन्यसनादेव सिद्धिं समधिगच्छति ॥4॥"],
                romanSloka: ["na karmaṇām anārambhān naiṣkarmyaṁ puruṣo’śnute", "na ca sannyasanād eva siddhiṁ samadhigacchati"],
                wordsMeaning: [
                    {
                        sanskrit: "न, कर्मणाम्",
                        meaning: "नहीं, कर्मों के",
                    },
                    {
                        sanskrit: "अनारम्भात्",
                        meaning: "विमुख रहकर",
                    },
                    {
                        sanskrit: "नैष्कर्म्यम्",
                        meaning: "कर्म-फलों से मुक्ति",
                    },
                    {
                        sanskrit: "पुरुषः, अष्नुते",
                        meaning: "मनुष्य, प्राप्त करता है;",
                    },
                    {
                        sanskrit: "न, च",
                        meaning: "नहीं, और",
                    },
                    {
                        sanskrit: "संन्यसनात्",
                        meaning: "त्याग से",
                    },
                    {
                        sanskrit: "एव",
                        meaning: "केवल",
                    },
                    {
                        sanskrit: "सिद्धिम्",
                        meaning: "सफलता",
                    },
                    {
                        sanskrit: "समधि-गच्छति",
                        meaning: "पा लेता है",
                    }
                ],
                meaningInHindi: "मनुष्य न तो कर्म से विमुख रहकर (कर्म न करके) कर्मफल से मुक्ति पा सकता है और न कर्मों के त्यागमात्र से (शारीरिक संन्यास लेकर ) ही वह सिद्धि (पूर्णत्व) को प्राप्त होता है।",
                meaningInEnglish: "A man can neither get freedom from the fruits of action by remaining aloof from action, nor does he attain the state of divine consciousness  only by renunciation of actions (physical renunciation).",
                meaningInGujrati: "",
                meaningInMarathi: "",
            },
            {
                slokaNumber: 5,
                uvach: "",
                sanskritSloka: ["न हि कश्चित्क्षणमपि जातु तिष्ठत्यकर्मकृत् ।", "कार्यते ह्यवशः कर्म सर्वः प्रकृतिजैर्गुणैः ॥5॥"],
                romanSloka: ["na hi kaścit kṣaṇam api jātu tiṣṭhaty-akarmakṛt", "kāryate hy-avaśaḥ karma sarvaḥ prakṛti-jair guṇaiḥ"],
                wordsMeaning: [
                    {
                        sanskrit: "न, हि",
                        meaning: "नहीं, निश्चय ही",
                    },
                    {
                        sanskrit: "कश्चित्, क्षणम्",
                        meaning: "कोई, क्षण के लिए",
                    },
                    {
                        sanskrit: "अपि, जातु",
                        meaning: "भी, सदैव",
                    },
                    {
                        sanskrit: "तिष्ठति, अकर्म",
                        meaning: "रह सकता है, कृत बिना कर्म",
                    },
                    {
                        sanskrit: "कार्यते, हि",
                        meaning: "कर्म करने के लिए, निश्चय ही",
                    },
                    {
                        sanskrit: "अवशः",
                        meaning: "बाध्य होकर",
                    },
                    {
                        sanskrit: "कर्म, सर्वः",
                        meaning: "कर्म, समस्त",
                    },
                    {
                        sanskrit: "प्रकृति-जैः",
                        meaning: "प्रकृति के गुणों से उत्पन्न",
                    },
                    {
                        sanskrit: "गुणैः",
                        meaning: "गुणों के द्वारा",
                    },
                ],
                meaningInHindi: "कोई भी मनुष्य क्षणमात्र  के लिए भी अकर्मा (बिना कर्म किए) नहीं रह सकता। क्योंकि मनुष्य प्रकृतिजन्य गुणों (प्रकृति द्वारा उत्पन्न तीन गुण - सत, रज, तम) के अनुसार कर्म करने के लिए विवश होते हैं।",
                meaningInEnglish: "No one can remain inaction (without doing action) even for a moment. Indeed, all living beings are forced to engage in action due to the influence of the modes of material nature.",
                meaningInGujrati: "",
                meaningInMarathi: "",
            },
            {
                slokaNumber: 6,
                uvach: "",
                sanskritSloka: ["कर्मेन्द्रियाणि संयम्य य आस्ते मनसा स्मरन् ।", "इन्द्रियार्थान्विमूढात्मा मिथ्याचारः स उच्यते ॥6॥"],
                romanSloka: ["karmendriyāṇi saṁyamya ya āste manasā smaran", "indriyārthān vimūḍhātmā mithyācāraḥ sa ucyate"],
                wordsMeaning: [
                    {
                        sanskrit: "कर्मेन्द्रियाणि",
                        meaning: "समस्त कर्म इन्द्रियोंको",
                    },
                    {
                        sanskrit: "संयम्य, यः",
                        meaning: "नियंत्रित (रोककर) करके, जो",
                    },
                    {
                        sanskrit: "आस्ते",
                        meaning: "रहता है",
                    },
                    {
                        sanskrit: "मनसा, स्मरन्",
                        meaning: "मन में, चिन्तन",
                    },
                    {
                        sanskrit: "इन्द्रिय-अर्थात",
                        meaning: "इन्द्रिय विषय",
                    },
                    {
                        sanskrit: "विमूढ-आत्मा",
                        meaning: "भ्रमित जीव",
                    },
                    {
                        sanskrit: "मिथ्या-आचार:",
                        meaning: "ढोंग",
                    },
                    {
                        sanskrit: "सः, उच्यते",
                        meaning: "वे, कहलाते हैं",
                    },
                ],
                meaningInHindi: "जो कर्मेन्द्रियों (सम्पूर्ण इन्द्रियों) को हठपूर्वक रोककर मन से इन्द्रिय विषयों (भोगों) का चिन्तन करता रहता है, वह मूढ़ बुद्धि वाला मनुष्य मिथ्याचारी (दम्भी) कहा जाता है। अर्थात् जो बाह्य इन्द्रियों को वश में करते हुए भी मन से भोगों का चिन्तन करता है, वह मूर्ख और कपटी कहा जाता है।",
                meaningInEnglish: "One who controls the external senses yet mentally dwells upon the sense-objects is said to be foolish and hypocritical.",
                meaningInGujrati: "",
                meaningInMarathi: "",
            },
            {
                slokaNumber: 7,
                uvach: "",
                sanskritSloka: ["यस्त्विन्द्रियाणि मनसा नियम्यारभतेऽर्जुन ।", "कर्मेन्द्रियैः कर्मयोगमसक्तः स विशिष्यते ॥7॥"],
                romanSloka: ["yas tv-indriyāṇi manasā niyamyārabhate’rjuna", "karmendriyaiḥ karma-yogam asaktaḥ sa viśiṣyate"],
                wordsMeaning: [
                    {
                        sanskrit: "यः, तु",
                        meaning: "जो, लेकिन",
                    },
                    {
                        sanskrit: "इन्द्रियाणि, मनसा",
                        meaning: "इन्द्रियाँ, मन से",
                    },
                    {
                        sanskrit: "नियम्य",
                        meaning: "नियत्रित करना",
                    },
                    {
                        sanskrit: "आरम्भते",
                        meaning: "प्रारम्भ करता है",
                    },
                    {
                        sanskrit: "अर्जुन",
                        meaning: "अर्जुन",
                    },
                    {
                        sanskrit: "कर्म-इन्द्रियैः",
                        meaning: "कर्म इन्द्रियों द्वारा",
                    },
                    {
                        sanskrit: "कर्म",
                        meaning: "कर्मयोग",
                    },
                    {
                        sanskrit: "असक्तः",
                        meaning: "आसक्ति रहित",
                    },
                    {
                        sanskrit: "सः-विशिष्यते",
                        meaning: "वे श्रेष्ठ हैं",
                    }
                ],
                meaningInHindi: "हे अर्जुन! जो पुरुष मन से इन्द्रियों को वश में करके अनासक्त होकर (निष्काम भावसे)  कर्मेंन्द्रियों से कर्मयोग का आचरण करता है वह श्रेष्ठ है।",
                meaningInEnglish: "O Arjun! Person who controls the senses with the mind and engages them in karma-yoga (the path of selfless action) without attachment is superior",
                meaningInGujrati: "",
                meaningInMarathi: "",
            },
            {
                slokaNumber: 8,
                uvach: "",
                sanskritSloka: ["नियतं कुरु कर्म त्वं कर्म ज्यायो ह्यकर्मणः ।", "शरीरयात्रापि च ते न प्रसिद्धयेदकर्मणः ॥8॥"],
                romanSloka: ["niyataṁ kuru karma tvaṁ karma jyāyo hy-akarmaṇaḥ", "śarīra-yātrāpi ca te na prasiddhyed akarmaṇaḥ"],
                wordsMeaning: [
                    {
                        sanskrit: "नियतं कुरु कर्म",
                        meaning: "वैदिक निर्धारित कर्म कर",
                    },
                    {
                        sanskrit: "त्वम्, कर्म",
                        meaning: "तुम, कर्म करना;",
                    },
                    {
                        sanskrit: "ज्यायः, हि",
                        meaning: "श्रेष्ठ, निश्चय ही",
                    },
                    {
                        sanskrit: "अकर्मणः",
                        meaning: "निष्क्रिय रहने की अपेक्षा",
                    },
                    {
                        sanskrit: "शरीर, यात्रा",
                        meaning: "शरीर का, पालन पोषण",
                    },
                    {
                        sanskrit: "अपि, च, ते",
                        meaning: "भी, भी, तुम्हारा",
                    },
                    {
                        sanskrit: "प्रसिद्धयेत्, अकर्मण",
                        meaning: "संभव न होना, निष्क्रिय",
                    }
                ],
                meaningInHindi: "तुम नियत कर्म करो (निर्धारित वैदिक कर्म),  क्योंकि (अकर्म) कर्म न करने की अपेक्षा कर्म करना श्रेष्ठ है तथा कर्म न करने से तेरा शरीर-निर्वाह भी सिद्ध नहीं होगा, अर्थात् कर्म का त्याग करने से तुम्हारे शरीर का भरण पोषण संभव नहीं होगा।",
                meaningInEnglish: "You should perform your prescribed duties, since action is better than inaction. You cannot maintain your existence without action.",
                meaningInGujrati: "",
                meaningInMarathi: "",
            },
            {
                slokaNumber: 9,
                uvach: "",
                sanskritSloka: ["यज्ञार्थात्कर्मणोऽन्यत्र लोकोऽयं कर्मबन्धनः ।", "तदर्थं कर्म कौन्तेय मुक्तसङ्गः समाचर ॥9॥"],
                romanSloka: ["yajñārthāt karmaṇo’nyatra loko’yaṁ karma-bandhanaḥ", "tad-arthaṁ karma kaunteya mukta-saṅgaḥ samācara"],
                wordsMeaning: [
                    {
                        sanskrit: "यज्ञ-अर्थात",
                        meaning: "यज्ञ के निमित्त किए जाने वाले कर्म",
                    },
                    {
                        sanskrit: "कर्मणः",
                        meaning: "कर्म की अपेक्षा",
                    },
                    {
                        sanskrit: "अन्यत्र",
                        meaning: "अन्यथा",
                    },
                    {
                        sanskrit: "लोक:",
                        meaning: "भौतिक संसार",
                    },
                    {
                        sanskrit: "अयम्",
                        meaning: "यह",
                    },
                    {
                        sanskrit: "कर्मबन्धनः",
                        meaning: "किसी के कर्मों के बन्धन",
                    },
                    {
                        sanskrit: " तत्, अर्थम्",
                        meaning: "वह, के लिए",
                    },
                    {
                        sanskrit: "कर्म",
                        meaning: "कर्म",
                    },
                    {
                        sanskrit: " कौन्तेय",
                        meaning: "कुन्तिपुत्र, अर्जुन",
                    },
                    {
                        sanskrit: "मुक्तसङ्गः",
                        meaning: "आसक्ति रहित",
                    },
                    {
                        sanskrit: "समाचर",
                        meaning: "ध्यान से कार्य करना",
                    }
                ],
                meaningInHindi: "यज्ञ (परमात्मा, कृष्ण के लिए) के लिये किये हुए कर्म के अतिरिक्त अन्य कर्मों में लगा हुआ यह पुरुष कर्मों द्वारा बंधता है, इसलिए हे कौन्तेय आसक्ति को त्यागकर उस यज्ञ के लिये ही कर्तव्य-कर्म कर। [यज्ञो वै विष्णुः - यज्ञ ही विष्णु है]",
                meaningInEnglish: "A man engaged in activities other than Yagya (for the Supreme, Krishna) is bound by Karma, therefore, O Kaunteya, give up attachment and do duty for the sake of Yagya. [Yagyo vai VishnuH - Yagya is Vishnu]",
                meaningInGujrati: "",
                meaningInMarathi: "",
            },
            {
                slokaNumber: 10,
                uvach: "",
                sanskritSloka: ["सहयज्ञाः प्रजाः सृष्ट्वा पुरोवाच प्रजापतिः ।", "अनेन प्रसविष्यध्वमेष वोऽस्त्विष्टकामधुक् ॥10॥"],
                romanSloka: ["saha-yajñāḥ prajāḥ sṛṣṭvā purovāca prajāpatiḥ", "anena prasaviṣyadhvam eṣa vo’stv iṣṭa kāmadhuk"],
                notClear: true,
                wordsMeaning: [
                    {
                        sanskrit: "सह, यज्ञाः",
                        meaning: "के साथ, यज्ञों",
                    },
                    {
                        sanskrit: "प्रजाः सृष्ट्वा",
                        meaning: "सृजन करना, मानव जाति",
                    },
                    {
                        sanskrit: "पुरा, उवाच",
                        meaning: "कहा, आरम्भ में",
                    },
                    {
                        sanskrit: "प्रजापतिः, अनेन",
                        meaning: "सृष्टिकर्त्ता, इससे",
                    },
                    {
                        sanskrit: "प्रसविष्यध्वम्",
                        meaning: "अधिक समृद्ध होना",
                    },
                    {
                        sanskrit: "एषः, वः",
                        meaning: "इन, तुम्हारा",
                    },
                    {
                        sanskrit: "अस्तु",
                        meaning: "होगा",
                    },
                    {
                        sanskrit: "इष्ट-काम-धुक",
                        meaning: "सभी वांछित पदार्थं प्रदान करने वाला",
                    }
                ],
                meaningInHindi: "प्रजापति (सृष्टिकर्त्ता) ने सृष्टि के आदि में यज्ञ सहित प्रजा का निर्माण कर कहा - इस यज्ञ द्वारा तुम वृद्धि को प्राप्त हो और यह यज्ञ तुम्हारे लिये इच्छित कामनाओं को पूर्ण करने वाला (इष्टकामधुक्) होवे।",
                meaningInEnglish: "In the beginning of creation, Brahmā created mankind along with the system of sacrifice and said, “Through this sacrifice may you prosper. May it fulfil all your desires.”",
                meaningInGujrati: "",
                meaningInMarathi: "",
            },
        ]
    }
};

export default Chapter3;